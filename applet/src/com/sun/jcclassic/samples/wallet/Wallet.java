...
public class Wallet extends Applet {
    ...
    private void credit(APDU apdu) {
        ...
        byte creditAmount1 = buffer[ISO7816.OFFSET_CDATA];
        byte creditAmount2 = buffer[ISO7816.OFFSET_CDATA+1];
        short creditAmount = (short)((short) creditAmount1 * 256 | (short) creditAmount2 );
...
private void update_pin(APDU apdu) {
        ...
        //oldpinVal = (short) 256 * bytesOldval * buffer[ISO7816.OFFSET_CDATA+1] ;
        //newpinVal = buffer[ISO7816.OFFSET_CDATA+1];
...
private void update_pin_v2(APDU apdu) {
        ...
        byte[] oldpinVal, newpinVal;
        oldpinVal= new byte[MAX_PIN_SIZE];
        newpinVal= new byte[MAX_PIN_SIZE];
               	
        
        byte oldpinLen = buffer[ISO7816.OFFSET_CDATA];        
        byte newpinLen = buffer[ISO7816.OFFSET_CDATA + (short)oldpinLen]; 
        
        byte index=0;
       //i = adresa primului byte al pinului vechi
        short i = ISO7816.OFFSET_CDATA+ 1 ;
        while ( //index< oldpinLen 
        		i <= (short) (ISO7816.OFFSET_CDATA + 2 + oldpinLen) )
        	{
        	oldpinVal[index] = buffer[i];
        	index++;
        	i++;
        	}
        	
        index=0;
        //i = adresa primului byte al pinului nou
        i = (short) (ISO7816.OFFSET_CDATA + (short) 2 + oldpinLen + 1 );
//        while (  i< (short) (ISO7816.OFFSET_CDATA + (short)2 + oldpinLen + (short)1 ) + (short) newpinLen ) 
//          	{
//           	newpinVal[index] = buffer[i];
//           	index++;
//           	i++;
//           	}
        
        // Ar trebui sa mai verific daca lungimea depaseste 
